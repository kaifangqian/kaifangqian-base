...
//            }
            reSignConfirmService.save(reSigner.getId(),reId,SignerTypeEnum.RECEIVER_PERSONAL.getCode(),signer.getAgreeSkipWillingness(),signer.getVerifyType(),signer.getPersonalSignAuth());
        }else if(SignerTypeEnum.SENDER.getCode().equals(signer.getSignerType())){
            reSignConfirmService.save(reSigner.getId(),reId,SignerTypeEnum.SENDER.getCode(),signer.getAgreeSkipWillingness(),signer.getVerifyType(),signer.getPersonalSignAuth());
        }else if(SignerTypeEnum.RECEIVER_ENT.getCode().equals(signer.getSignerType())){
            reSignConfirmService.save(reSigner.getId(),reId,SignerTypeEnum.RECEIVER_ENT.getCode(),signer.getAgreeSkipWillingness(),signer.getVerifyType(),signer.getPersonalSignAuth());
        }
    ...
    public void updateSigner(ReSaveDataSigner signer,String reId){
        ...
        if(SignerTypeEnum.RECEIVER_PERSONAL.getCode().equals(signer.getSignerType())){
            //个人
            updateConfirm(reSigner.getId(),reId,SignerTypeEnum.RECEIVER_PERSONAL.getCode(),signer.getConfirmType(),signer.getAgreeSkipWillingness(),signer.getVerifyType(),signer.getPersonalSignAuth());
        }else if(SignerTypeEnum.SENDER.getCode().equals(signer.getSignerType())){
            updateConfirm(reSigner.getId(),reId,SignerTypeEnum.SENDER.getCode(),signer.getConfirmType(),signer.getAgreeSkipWillingness(),signer.getVerifyType(),signer.getPersonalSignAuth());
        }else if(SignerTypeEnum.RECEIVER_ENT.getCode().equals(signer.getSignerType())){
            updateConfirm(reSigner.getId(),reId,SignerTypeEnum.RECEIVER_ENT.getCode(),signer.getConfirmType(),signer.getAgreeSkipWillingness(),signer.getVerifyType(),signer.getPersonalSignAuth());
        }
        //新增内部节点
        addSenderList(signer.getAddSenderList(),reSigner.getId(),reId);
        //更新内部节点
...
//                }
                reSignConfirmService.save(reSender.getId(),reId, sender.getSignerType(), sender.getAgreeSkipWillingness(), sender.getVerifyType(), sender.getPersonalSignAuth());
            }
        }
    }
    ...
    public void updateSenderList(List<ReSaveDataSender> updateSenderList,String singerId,String reId){
        if(updateSenderList != null && updateSenderList.size() > 0) {
            for (ReSaveDataSender sender : updateSenderList) {
                ...
                updateConfirm(reSender.getId(),reId,reSender.getSenderType(),sender.getConfirmType(),sender.getAgreeSkipWillingness(),sender.getVerifyType(),sender.getPersonalSignAuth());
            }
        }
    }
    ...
    public void updateConfirm(String signerId,String signReId,Integer signerType,Integer confirmType,Integer isFastSign,String verifyType,String personalSignAuth){
//        if(confirmType == 1){
...
//        }
        SignReSignConfirm reSignConfirm = reSignConfirmService.getByParam(signerId, signReId);
        if(reSignConfirm == null){
            reSignConfirmService.save(signerId,signReId,signerType,isFastSign,verifyType,personalSignAuth);
        }else{
